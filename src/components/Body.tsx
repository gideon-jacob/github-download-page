...
import { Box, Button, IconButton, Stack, Typography, useMediaQuery, CircularProgress } from "@mui/material";
...
function Preview() {
  ...
  const screenshots = window.siteConfiguration.application.screenshots || [];
...
function Body() {
  ...
  // Fetch GitHub data
  const { repository, latestRelease, loading } = useGitHubData(
    ...
    <Box sx={{
      ...
      <Box sx={{ flex: 1, width: greaterThan960 ? '0' : '100%' }}>
        ...
        <Typography variant="body1" mt={2}>
          {repository?.description || window.siteConfiguration.application.description}
        </Typography>
      </Box>
      <Box sx={{ width: greaterThan960 ? '300px' : '100%' }}>
        ...
        <Stack>
          ...
          <SideItem 
            name="Updated on" 
            value={loading ? <CircularProgress size={16} /> : (
              repository ? githubApi.formatDate(repository.updated_at) : window.siteConfiguration.application.info.updatedOnString ?? 'Unknown'
            )} 
          />
          <SideItem 
            name="Released on" 
            value={loading ? <CircularProgress size={16} /> : (
              latestRelease ? githubApi.formatDate(latestRelease.published_at) : window.siteConfiguration.application.info.releasedOnString ?? 'Unknown'
            )} 
          />
        </Stack>
      </Box>
